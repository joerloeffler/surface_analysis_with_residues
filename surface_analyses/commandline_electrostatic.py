...
def main(argv=None):
    ...
    ##### ADDED parser option here!
    parser.add_argument('--patch_res', type=str, default=None, help='CSV file to store patch type and residues.')
    parser.add_argument(
        '-c', '--patch_cutoff',
        type=float,
        nargs=2,
    ...
    patches = pd.DataFrame({
        ...
        'chain': chain_ids[closest_atom], ### added chain information here
    ...
    if args.patch_res:
        print(f"Writing patch residue assignments to {args.patch_res}")
        write_patch_residue_assignments(patches, args.patch_res)
...
### Function for additional output file
def write_patch_residue_assignments(df, filename, cols=['positive', 'negative']):
    data = [] #print("getting info")
    for column in cols:
        groups = df[df[column] != -1].groupby(column)
        for patch_id, group in groups:
            for res in group['residue'].unique():
                chain = group.loc[group['residue'] == res, 'chain'].iloc[0]
                data.append({
                    'patch_type': column,
                    'patch_number': patch_id,
                    'chain': chain,
                    'residue': res
                    })
